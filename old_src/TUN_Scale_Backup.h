...
/**
 * Convert midi note to hertz (Hz) using the default 12-tone equal tempermant mapping
...
/**
 * Convert midi note to cents using the default 12-tone equal tempermant mapping
 * provided by the MIDI specification.
 * @param  midiNote  MIDI note from 0 to 127.  If outside the range, @p midiNote will be clamped.
...
public:
  ...
  std::list<std::string>
      m_lstrCompositions; // Format: "Musician or Band|Album|Title|Year|Misc"
...
public:
  ...
  static const char *FormatSpecs() {
    return "http:\\\\www.mark-henning.de\\eternity\\tuningspecs.html";
  } // Never Change!
